...
function syntaxCheckWorkerNew(val, check = undefined, errorObj = {mustacheSyntax:{arr:[]}}, index = {},attr) {
    if (!check.Compile) {
...
function processAttr(val) {
    ...
    if (parsingMustache) {
        ...
        retObj.err = errorType[9] + "'{' bracket";
        retObj.errType = 9;
        retObj.errorIndex = mustacheIndex;
        return retObj;
    }
    if (!type) {
        type = "Normal Data"
            ...
            if (attrs.length !== 0) {
              node.attr = attrs.reduce(function (acc, attr) {
                ...
                let res = (0,_HandleMustache__WEBPACK_IMPORTED_MODULE_2__["default"])(value, check, errorObj, {
                  lineIndex,
                  colIndex:attr.attrStartIndex || colIndex
                },true);
                value = res.text;
...
function getSelectorType(selector){
	if(selector[0]=='.') { return {_attr:"class",_attrVal:selector.substring(1),single:1}; }
		...
		if(selector.search('=')!=-1){
			...
			return {_attr:attr,_attrVal:attrVal,single:1};
		}
		else {
			let attr = selector.substring(1,selector.length-1);
			return {_attr:attr,_attrVal:null,single:2};
		}
	};	
	return {_attr:selector,_attrVal:null,single:3};
}
...
class Node{
	...
	//.querySelectorAll()
	findAll(obj,listToCheck,dontCheckTemplate,result = []){
		obj = ((obj.tag == "template" && dontCheckTemplate && obj.tagName == "TEMPLATE") ? obj.content.child : obj.child);
		if(!obj){ return result;}
		for (let i=0; i<obj.length; i++){
			for(let lsItem of listToCheck){
				let {single,_attr,_attrVal} = lsItem;
				if(single == 1 && obj[i].attr && obj[i].attr[_attr] == _attrVal)
					{ result.push(obj[i]); break; }
				else if (single == 2 && obj[i].attr && this.find(obj[i].attr, _attr)) 
					{ result.push(obj[i]); break; }
				else if(single == 3 && obj[i].tag == _attr)
					{ result.push(obj[i]); break; }
			}
			this.findAll(obj[i],listToCheck,dontCheckTemplate,result);
		}
		return result;
	}
...
/******/ 	var __webpack_exports__ = __webpack_require__("./src/nodeStructClass.js");
/******/ 	Object(window["html-parser"]).nodeStructClass = __webpack_exports__["default"];
/******/ 	
/******/ })()
;