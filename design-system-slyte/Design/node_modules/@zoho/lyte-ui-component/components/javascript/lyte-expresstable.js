...
import {shouldIgnoreDisconnect} from "@slyte/component"
...
class LyteExpresstableComponent extends Component {
    ...
    didConnect() {
         ...
         this.$node.scrollTo = function( arg1, arg2 ,event){
            ...
            if( this._scrolled ){
                 ...
                 this.getActions("scroll").call( this,event,arg1,arg2, true );
               }.bind(this),20)
             }
    ...
    setFixedTable() {
        ...
        for(var i = 0 ; i < tableHeader.length ; i++ ) {
            if( tableHeader[i].fixed && tableHeader[i].fixed == "enable" && originalTableHeader[i].offsetWidth != 0 ) {
                this.$addon.arrayUtils(fixedTableHeader,'push',tableHeader[i])
            }
        }
            ...
            if( col!=undefined && ( col.nodeType === 1 || !isNaN( col ) ) ) {
                ...
                $L.fastdom.mutate( function() {
                    ...
                    if($L(sthList[colIndex]).hasClass('lyteFixedColumn')){
                        window.ofthList=that._originalDiv.getElementsByClassName('lyteFixedColumn')
                        window.fColIndex = Array.from( ofthList ).indexOf( sthList[colIndex] );
                        if(that._fixHeaderCount>=0){
                           var _colwrap = that._colwrap || that.$node.getElementsByClassName( 'lyteExpTableFixedColWrapper' ) [ 0 ];

                            window.fixothList = _colwrap.getElementsByTagName('lyte-exptable-th') ;
                            var w=fixothList[fColIndex].style.width  
                           fixothList[fColIndex].style.width = that._widthList[colIndex].width +"px";
                           window._fcolwrap = that.$node.getElementsByClassName( 'lyteExpTableFixedColHeader' ) [ 0 ];
                           window.ffixothList = _fcolwrap.getElementsByTagName('lyte-exptable-th') ; 
        ...
        if( scrollLeft != this._prevScrollLeft || ignore ) {
            ...
            for( var n = columns.length - 1; n >= 0; n-- ){
                ...
                if( columns[ n ]._off.width == 0 && n-1 >= 0 && this.rtlfunc( 'right', columns[ n-1 ]._off, wWidth ) <= this.rtlfunc( 'right', originalFixed[ n-1 ]._off, wWidth ) + ( this._isIE ? 2 : 0 ) ){
                    ...
                    this._fixHeaderCount--;
                    this.setVisibleFixHeader()
                    this.$addon.arrayUtils( columns, 'removeAt', n );
                }
                    ...
                    for( var k = columns.length - 1; k >= n; k-- ){
                        ...
                        this.$addon.arrayUtils( columns, 'removeAt', k );
                    }
    ...
    horiScrollResize(dummyDiv, ww) {
        ...
        if( isEvt ){
            ...
            check2 = this.rtlfunc.call( this, 'clientX', dummyDiv, ww ) <= Math.max( 0, this.rtlfunc( 'left', this._thisBccr, window.innerWidth ) );
        } else {
    ...
    hideRows(bcr) {
        ...
        for( var i = 1; i < originalRows.length; i++ ){
            ...
            if( _bcr.bottom < tValue ){
                ...
                this.$addon.arrayUtils( visible, 'push', originalRows.slice( i, i + 10 ) );
                this.$addon.arrayUtils( topRows, 'push', originalRows.splice( i + 10 ) );
                if( !isSticky ){
                    this.$addon.arrayUtils( visible, 'push', fakerows.slice( i, i + 10 ) );
                    this.$addon.arrayUtils( topRows, 'push', fakerows.splice( i + 10 ) );
                }
                break;
            } else {
                visible.push( row );
    ...
    sortablemousedown(ev) {
        ...
        if( is_sticky && td.getAttribute('fixed') === "enable"){
            // td.removeAttribute('fixed')
        }
    ...
    sortmousemove(ev, allow, pos) {
        ...
        closestTd = closestTd || this.getTd( td, inc, pos ),
        ...
        if( closestTd && this.getMethods( 'onBeforeInterChange' ) ){
            ...
            if( interchangeprevent && newone ){
                this.$addon.arrayUtils( this._affectedIndex, 'removeAt', this._affectedIndex.indexOf( index ) );
            }
        }
        if( prevtable ){
    ...
    sorthorizontalscroll(bcr, sL) {
         ...
         if( check1 || check2 ){
            ...
            this._measure = $L.fastdom.measure( function(){
                delete this._measure;
                this.getActions("scroll").call( this );
    ...
    sortmouseup(evt) {
        ...
        if( this._moved ){
            ...
            if( !failed ){
                if( index != newindex ){
                    this._prevent = true;
                    var tableHeader = this.data.tableHeader,
                    current = this.$addon.arrayUtils( tableHeader, 'removeAt', index );
                    this.$addon.arrayUtils( tableHeader, 'insertAt', newindex, current );
                    var header = this.data.ltPropHeader;
                    current = this.$addon.arrayUtils( header, 'removeAt', index );
                    this.$addon.arrayUtils( header, 'insertAt', newindex, current );
                    delete this._prevent;
                ...
                $L.fastdom.mutate( function()  {
                    ...
                    headerCells.forEach( function( cell, index )  {
                        ...
                        if( intersection ){
                            ...
                            // ignoreWidth =  0
                            // var _leftminusWidth = accumulatedLeft - ignoreWidth
                            ...
                            // this.removeFixedColumn( rows, index, left, accumulatedWidth );
...
if( !window._lyteUiUtils.registeredCustomElements[ 'lyte-exptable-th' ] ) {
    ...
    class LyteExptableThCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
        ...
        attributeChangedCallback(attr, oldVal, newVal) {
            ...
            if (attr == 'fixed') {
            var scrollingDiv = this.$node;
             while(scrollingDiv.tagName != 'DIV')
                {
                    scrollingDiv = scrollingDiv.parentElement;
                }  
            if (newVal == 'enable') {
                this.$node.classList.add('lyteFixedColumn');
            } else {
                this.$node.classList.remove('lyteFixedColumn');
            }
            ...
            if (newVal == 'enable') {
              ...
              this.$node.appendChild(tabHead);
            } else {
              var tabHead = this.$node.getElementsByTagName('lyte-exptablehead-resize')[ 0 ];
              if (tabHead) {
                this.$node.removeChild(tabHead);
              }
            }
          }
        }
        ...
        checkIntersection(arg) {
                var table = $L( this.$node ).closest( 'lyte-expresstable' ).get( 0 );
                if( table.getData('ltPropStickyTable') ){
                    if( arg && !this.$node._horizontalIntersectionDiv ){
                        this.createIntersection( table );
                    } else if( !arg && this.$node._horizontalIntersectionDiv ){
                        var intersection = this.$node._horizontalIntersectionDiv;
                        this.removeIntersection( intersection, table );
                        table.component.removeSticky( this.$node );
                        table.component.removeFixedClass( this.$node );
        ...
        createIntersection(table) {
                ...
                this.$node._horizontalIntersectionDiv = div;
                div._cell = this.$node;
        ...
        removeIntersection(intersection, table) {
                ...
                delete this.$node._horizontalIntersectionDiv;
            }
...
if( !window._lyteUiUtils.registeredCustomElements[ 'lyte-exptable-tr' ] ) {
    ...
    class LyteExptableTrCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
        ...
        connectedCallback() {
            ...
            table = this.$node.closest ? this.$node.closest( query ) : $L( this.$node ).closest( query ).get( 0 );

            if( table && table.ltProp( 'role' ) && table.ltProp( 'yield' ) ){
                table.component.update_aria( this.$node );
            }
        }
    }
