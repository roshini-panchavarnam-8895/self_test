...
class Serializer extends Service{
    ...
    static getResponse(db,resp,def,type,key,urlObj,xhr,customData,opts,argsObj){
        ...
        if(doNormalize){
            ...
            if(len){			
                ...
                */
                baseSerz.normalize(db, def, type, payLoad, key, xhr ? xhr.status : xhr, urlObj.headers, customData, opts, urlObj, argsObj);
            }
            return payLoad;
        }
        else{
    ...
    static serializeRecords(db,schema,data,records,urlObj,type,customData,argsObj,partial){
        ...
        PerfomanceLog(db.lyte , argsObj.__reqId__ , "["+(Array.isArray(data) ? data.length : 1 )+"]"+type ,argsObj?argsObj.schemaName:undefined)
    ...
    static buildJSON(db, def, type, payLoad, id, status, urlObj, customData, qP, argsObj){
        ...
        if(isGet || (realData && realData[name])){
            ...
            //@Slicer.developmentStart
            PerfomanceLog(db.lyte , argsObj.__reqId__ , "["+(Array.isArray(recs) ? recs.length : 1) +"]"+"processEntity" ,argsObj?argsObj.schemaName:undefined)
            //@Slicer.developmentEnd
            if(scope){
    ...
    static normalize(db,def,type,payLoad,id,status,headers,customData,opts,urlObj,argsObj){
        ...
        if(scope){
            //@Slicer.developmentStart
            PerfomanceLog(db.lyte , argsObj.__reqId__ , "["+(Array.isArray(realData) ? realData.length : 1) +"]"+"processEntity" ,argsObj?argsObj.schemaName:undefined)
            //@Slicer.developmentEnd
