...
class Shadow extends Service{
    ...
    insertBefore(registryInstance, referenceNode, newNode, parentNode,obj) {
        ...
        if(shadowParent && hostEleParent && referenceNode && hostEleParent != this.getHostElement(referenceNode.getRootNode())){
            ...
            var a ={"type":"insertBefore","shadowParent":hostEleParent,"outlet":parentNode,"element":newNode ,"refNode": referenceNode ? referenceNode : null,"nonShadowParent":nonShadowParent}
            this.addElements(a,registryInstance);
        }
        else{
    ...
    attachStyleToParentShadow(comp,shadowParent,content){
        ...
        if(comp._compClass._style && shadowParent._duplicateStyle.indexOf(tagName)==-1){
            ...
            this.prependStyle(shadowParent, style, comp);
            if(shadowParent._sw.length > 0){
                ...
                shadowParent._sw.forEach(function(item,index){
                    if(item.shadowRoot._duplicateStyle.indexOf(tagName) == -1){
                        ...
                        style.setAttribute("lyte-id","shadow-style-"+tagName);
                        self.prependStyle(item.shadowRoot, style, comp);
                    }
                });
            }
        }
    ...
    prependStyle(shadowParent ,style, comp, shadowWrapper){
        var nodes = shadowParent.querySelectorAll("[lyte-id^='shadow-']");
        var last = nodes[nodes.length- 1];
        _LC.addLink(comp, shadowParent, style , {position : "before", refNode : last ? last.nextSibling : shadowParent.childNodes[0], shadowWrapper : shadowWrapper});
        // if(last){
        ...
        if(parentStyleList.length > 0){
            parentStyleList.forEach(function(item,index){
                ...
                if(finalshadow._duplicateStyle.indexOf(item) == -1 && compIns._style){
                    ...
                    // sty._id = id;
                    let sdWp;
                    if(finalshadow.host && finalshadow.host.localName == "shadow-wrapper"){
                        sdWp = finalshadow.host;
                    }
                    self.prependStyle(finalshadow, sty, registryInstance, sdWp);
                }
            })
        }
    ...
    removeLink(comp,registryIns){
        if(comp.__link){
            comp.__link.forEach(function(link){
                let app = _LC.getNearestParentApp(registryIns);
                if(app.removeLink){
                    app.removeLink.apply(app,[link]);
                    return;
                }
            })
            comp.__link = null;
    ...
    addLink(comp,outlet,style,options,nearByApp){
        let actComp = comp;
        let sdWp;
        if(style.localName == "link"){
            if(options && options.shadowWrapper){
                sdWp = options.shadowWrapper;
                options.shadowWrapper = null;
                delete options.shadowWrapper;
            }
            if(comp && comp.localName == "lyte-yield"){
                comp = comp._callee;
            }
            let app = nearByApp || _LC.getNearestParentApp(comp.isComponentRegistry && comp.isComponentRegistry() ? comp : comp.component);
            if(app.addLink){
                if(actComp){
                    let linkComp = sdWp ? sdWp : actComp;
                    if(!linkComp.__link){
                        linkComp.__link = [];
                    }
                    linkComp.__link.push(style);
                }
                app.addLink.apply(app,[outlet,style,options]);
                return;
            }
        }
        Lyte._renderPosition(outlet, style, options);
    }
    destroyRef(comp){
        let registryIns = comp.$registry;
        if(comp.localName == "lyte-yield"){
            registryIns = comp._callee.$registry;
        }
        this.removeLink(comp, registryIns)
        if(comp.shadowRoot){
            ...
            if(shadowWrapper && shadowWrapper.length > 0){
                for(var i=0;i<shadowWrapper.length;i++){
                    ...
                    index > -1 ? this.shadowList.$splice(index,1):null;
                    if(shadowWrapper[i].__link){
                        this.removeLink(shadowWrapper[i], registryIns);
                    }
    ...
    newShadowComp(comp,shadowParent){
        ...
        if(shadowParent && _LC.directive.getTransitionArg(comp,"shadow-deep") == undefined && _LC.directive.getTransitionArg(pNode,"shadow-deep") ){
            if(!comp._specialAttributeDetails){
                comp._specialAttributeDetails = [];
            }
            comp._specialAttributeDetails.push(_LC.directive.getTransitionArg(pNode,"shadow-deep",true) )
