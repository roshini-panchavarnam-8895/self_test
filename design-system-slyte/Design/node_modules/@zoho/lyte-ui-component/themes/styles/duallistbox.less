...
.lyteListboxLeftPanel {
	.lyteListboxLeftPanelMixin();
	display: flex;
	flex-direction: column;
	@{property-margin-right}: @margin-right;
}
.lyteListboxRightPanel {
	.lyteListboxRightPanelMixin();
...
.lyteListBoxRightElement:hover {
	.ListBoxLeftRightElementHoverMixin();
	background: @bg;
}
.lyteListboxYield {
	.lyteListboxYieldMixin();
	max-width: @max-w;
	overflow: auto;
	flex: 1;
	@{property-margin-right}: @margin-right;
...
.lyteListBoxNoResultsFound {
	.lyteListBoxNoResultsFoundMixin();
	width: @w;
	height: @h;
	display: flex;
	justify-content: center;
...
@keyframes lytelistboxBgAnim {
	from {
		.lytelistboxBgAnimFromMixin();
		background: @bg;
		color: #333;
	}
	to {
		.lytelistboxBgAnimToMixin();
		background: @bg;
	}
}
