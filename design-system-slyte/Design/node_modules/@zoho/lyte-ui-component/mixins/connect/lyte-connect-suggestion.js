...
class LyteConnectSuggestionMixin extends Mixin {
    create_conn(side, svg_details, check_overlap, undo_bool) {
    ...
    remove_data(position, avail_pos) {
        position.forEach(function (item) {
            window.index = avail_pos.indexOf(item);
            if (window.index == -1) {
                return;
            }
            avail_pos.splice(window.index, 1);
        });
    }

    avail_dec(__this, element) {
    ...
    element_selections(_this, cur_id, cur_element, type) {
        var avail_pos = $L.extend([], _this.getData('ltPropSuggestionData')),
            ...
            overlap_check = function (__this) {
                ...
                avail_pos = avail_pos.filter(function (item) {
                    ...
                    for (window.key in all_positions) {
                        if (window.key != cur_id) {
                            var new_pos = all_positions[window.key].position,
        ...
        elements.forEach(function (item, index) {
            if (type == "add" && avail_pos.includes(item.direction)) {
                ...
                this.set_suggest_data(item, ["class", "type", "svg_id"], [(item.class.replace(" lyteElementSuggestedDisplay", "")), "none", "none"]);
                destroyCallBck = true;
    ...
    get_suggest_details(_this, index, hover, event) {
        ...
        if (type == "create" || type == "create_more") {
            ...
            this.$addon.arrayUtils(this.getData('fakeData'), 'push', [{ "id": trg_id, "position": src_positions, "class": 'lyteConnectSuggestedItem' }]);
        }
    ...
    set_suggest_data(object, param, class_css) {
        for (var i = 0; i < param.length; i++) {
            this.$addon.objectUtils(object, "add", param[i], class_css[i]);
        }
    }

    remove_connection(_this, index, conn_item) {
        var suggest_data = _this.getData("suggestionData")[index],
        ...
        if (!suggest_data.type.connect && this.getData('fakeData').length > 0) {
            this.$addon.arrayUtils(this.getData('fakeData'), 'pop');
        }
            ...
            element_selection: function (_this, type) {
                ...
                if (type == "add") {
                    if (this.getMethods('onBeforeSuggestionCreate')) {
                        var ret = this.executeMethod('onBeforeSuggestionCreate', _this.$node);
                ...
                $L.fastdom.measure(function () {
                    ...
                    $L.fastdom.mutate(function () {
                        if (connect) {
                            ...
                            this.$node.disConnect(svg_el, false, true);
                            var trg_element = lyteSelf.$addon.arrayUtils(this.getData('fakeData'), 'pop');
                            delete trg_element.class;
                        }
                        if (this.getMethods('onBeforeSuggestionClick') && this.executeMethod('onBeforeSuggestionClick', trg_element || this.get_element(svg_details.target.replace("#", "")), svg_details, _this.$node, event, this.$node) == false) {
                            removeData();
                            return;
                        }
                        if (!connect) {
...
export { LyteConnectSuggestionMixin };
