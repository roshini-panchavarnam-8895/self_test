...
import './lyte-tree.js';
import { objectUtils, arrayUtils } from "../../../../@slyte/component/index.js";
...
class LyteTreeComponent extends Component {
  constructor() {
    super();
  }
  ...
  data(arg1) {
    return Object.assign(super.data({
      ...
      'ltPropChildrenValue': prop('string', {
        'default': 'children'
      }),
      ...
      'ltPropLeafContainer': prop('string', {
        'default': 'lyteTreeNoChildContainer'
      }),
      ...
      'ltPropWrapperOpenClass': prop('string', {
        'default': ''
      }),
      ...
      'ltPropWrapperCloseClass': prop('string', {
        'default': ''
      }),
      ...
      'ltPropLeafNodeClass': prop('string', {
        'default': 'lyteTreeHasNoChild'
      }),

      'ltPropMaxLevel': prop('number', {
        'default': 35
      }),

      "ltPropStructureType": prop('string', {
        'default': 'file'
      }),

      "ltPropToggleAnimation" : prop('boolean' , {
        'default' : true
      }),
      ...
      "ltPropAllowExternalImport" : prop('boolean' , {
        default : false
      }),
      "ltPropIgnoreMaxLevel" : prop('boolean' , {
        default : false
      }),
      ...
      "ariaLiveBoolean" : prop('boolean' , {
        'default' : false
      }),


      'treeHeight': prop('number', {
        'default': 0
      }),
      'ltPropStateAttr': prop('string', {
        'default': ''
      }),
      'ltPropCssLevelVariable' : prop('string' , {
        'default' : 'treeLevel'
      }),


			'dataType' : prop('string' , {
				default : 'array'
			}),
			'lyteTreeArrayOpened' : prop('boolean' , {
				default : false
			}),
			'lyteTreeJsonOpened' : prop('boolean' , {
				default : false
			}),
      'lyteTreeArrayLength' : prop('number' , {
        default : 0
      }),
      'arrayOpened' : prop('boolean' , {
        default : false
      }),
      'jsonOpened' : prop('boolean' , {
        default : false
      }),

      'heightTransArr': prop('array', {
        default: []
      }),

      'collapsedAll': prop('boolean', {
        default: false
      }),
      ...
      'indexVar': prop('string', {
        default: ''
      }),
      'currentSortObject' : prop('object'),
      'currentClickedBody' : prop('string' , {
        default : ''
      }),
      'treeDimension' : prop('object' , {
        default : {}
      }),
      'contentDimension' : prop('object' , {
        default : {}
      }),
      'initialClientY' : prop('number' , {
        default : 0
      }),
      'currentBodyDimension' : prop('object'),
      'previousSortIndex' : prop('string'),
      'currentSortIndex' : prop('string'),
      'triggerSortable' : prop('boolean' , {
        default : false
      }),
      'exeDrop' : prop('boolean' , {
        default : false
      }),
      'initialTop' : prop('number' , {
        default : 0
      }),
      'initialLeft' : prop('number' , {
        default : 0
      }),
      'stopScroll' : prop('boolean' , {
        default : true
      }),
      'scrollTriggered' : prop('boolean' , {
        default : false
      }),
      'dropZoneDim' : prop('object' , {
        default : {}
      }),
      'indexObj' : prop('string' , {
        default : ""
      })
    }), arg1);
  }
        ...
        function openCollapsed(treeData){
          for(var i=0;i<treeData.length;i++){
            if(treeData[i].collapsed === true){
              lyteSelf.$addon.objectUtils(treeData[i] , 'add' , 'collapsed' , false)
            }
    ...
    this.$node.openTree = function(targetElem , param , openAllNodes){
      ...
      if(param && param.changeCollapsed){
        ...
        if(data[currentTree.getData('ltPropChildrenValue')] && data[currentTree.getData('ltPropChildrenValue')].length > 0){
          lyteSelf.$addon.objectUtils(data , 'add' , 'collapsed' , false)
  ...
  mouseMoveFunction(eve, external) {
    if(external){
    ...
    if(Math.abs(treeComponent.data.initialClientY - eve.clientY) > 5){
      ...
      if(treeContent && (beforeDrag !== false)){
        ...
        if(eve.clientY < (treeContentDim.top + topHeight)){
          ...
          // bodyPlaceHolder
          if(!$L(treeContent).hasClass("lyteTreeSortableDropPlaceholderChild")) {
            $L(topTree).find('.lyteTreeSortableDropPlaceholderBottom').removeClass('lyteTreeSortableDropPlaceholderBottom')
            $L(topTree).find('.lyteTreeSortableDropPlaceholderTop').removeClass('lyteTreeSortableDropPlaceholderTop')
        ...
        if(treeComponent.data.exeDrop){
          ...
          if(allowDrop){
            if(eve.clientY < (treeContentDim.top + topHeight)){
              ...
              if(beforeDrop !== false){
                this.$addon.arrayUtils( prevDataArr , 'removeAt' , currentClickedBodyInd.charAt(currentClickedBodyInd.length-1) , 1 )
                if(sibInsertIndex < 0){
                  sibInsertIndex = 0;
                }
                this.$addon.arrayUtils( currentTreeArr , 'insertAt' , sibInsertIndex , treeComponent.getData('currentSortObject'));
              }
      
            } else if((eve.clientY > (treeContentDim.top + topHeight)) && (eve.clientY < (treeContentDim.top + midheight))){
              if(treeComponent.getData('currentClickedBody') !== $L(currentTreeBody).attr('data-index')){
                if(!currentTreeData[treeComponent.getData('ltPropChildrenValue')]){
                  this.$addon.objectUtils(currentTreeData , 'add' , treeComponent.getData('ltPropChildrenValue') , [])
                }
                callBackObj = {}
                ...
                if(beforeDrop !== false){
                  this.$addon.arrayUtils( prevDataArr , 'removeAt' , currentClickedBodyInd.charAt(currentClickedBodyInd.length-1) , 1 )
                  this.$addon.arrayUtils( currentTreeData[treeComponent.getData('ltPropChildrenValue')] , 'push' , treeComponent.getData('currentSortObject'))
    ...
    if(currentTreeData.collapsed === undefined ||
       currentTreeData.collapsed === true){
         ...
         if($L(th).find('lyte-tree-icon') && $L(th).find('lyte-tree-icon').length>0){
          ...
          this.$addon.objectUtils(currentTreeData , 'add' , 'collapsed' , false)
         ...
         this.$addon.objectUtils(currentTreeData , 'add' , 'collapsed' , true)
...
LyteTreeComponent._template = "<template tag-name=\"lyte-tree\" role=\"treeitem\"> <template _new=\"true\" value=\"{{ltPropStructureType}}\" is=\"switch\">   <template case=\"file\" is=\"case\"> <template items=\"{{ltPropData}}\" item=\"item\" index=\"index\" is=\"for\" _new=\"true\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(item.skipNode,'!')}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteTreeBodyDiv\" role=\"group\"> <lyte-tree-body data-index=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\" class=\"{{lyteUiTreeClassHelp(item.defaultState,item.collapsed,'lyteTreeBodyOpened','lyteTreeBodyClosed',ltPropWrapperOpenClass,ltPropWrapperCloseClass)}}\" data-level=\"{{lyteUiTreeLevelHelp(indexVar,index)}}\" style=\"{{lyteUiTreeLevelVariableHelp(indexVar,index,ltPropCssLevelVariable)}}\"> <div class=\"mainContainer lyteTreeMainContainer {{lyteUiTreeChildHelp(item,ltPropLeafContainer,ltPropChildrenValue)}} {{lyteUiTreeHasChildHelp(item,ltPropChildrenValue)}} {{lyteTreeMaxChild(indexVar,index,ltPropMaxLevel)}}\" role=\"{{lyteTreeAriaRole(item,ltPropChildrenValue)}}\"> <template _new=\"true\" value=\"{{lyteTreeTypeHelp(ltPropSortable,ltPropAllowExternalImport)}}\" is=\"switch\">   <template case=\"withExternal\" is=\"case\"> <div class=\"lyteTreeSortableElement\" ondragenter=\"{{action('dragEnterFunction',event,this)}}\" ondragover=\"{{action('dragOverFunction',event,this)}}\" ondragleave=\"{{action('dragLeaveFunction',event,this)}}\" ondrop=\"{{action('dropFunction',event,this)}}\"> <lyte-yield tabindex=\"{{lyteTreeAriaTabIndexHelp(indexVar,index)}}\" collapsed=\"{{if(item.collapsed,'collapsed','')}}\" yield-name=\"content\" list-value=\"{{item}}\" list-index=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\" class=\"{{lyteUiTreeChildHelp(item,ltPropLeafNodeClass,ltPropChildrenValue)}}\"></lyte-yield> </div> </template><template case=\"onlyInternal\" is=\"case\"> <div class=\"lyteTreeSortableElement\"> <lyte-yield tabindex=\"{{lyteTreeAriaTabIndexHelp(indexVar,index)}}\" collapsed=\"{{if(item.collapsed,'collapsed','')}}\" yield-name=\"content\" list-value=\"{{item}}\" list-index=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\" class=\"{{lyteUiTreeChildHelp(item,ltPropLeafNodeClass,ltPropChildrenValue)}}\"></lyte-yield> </div> </template><template case=\"nonSortable\" is=\"case\"> <lyte-yield tabindex=\"{{lyteTreeAriaTabIndexHelp(indexVar,index)}}\" collapsed=\"{{if(item.collapsed,'collapsed','')}}\" yield-name=\"content\" list-value=\"{{item}}\" list-index=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\" class=\"{{lyteUiTreeChildHelp(item,ltPropLeafNodeClass,ltPropChildrenValue)}}\"></lyte-yield> </template></template> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(expHandlers(expHandlers(expHandlers(item[ltPropChildrenValue][&quot;length&quot;],'!==',0),'&amp;&amp;',expHandlers(item[ltPropChildrenValue],'!==',undefined)),'&amp;&amp;',expHandlers(item.collapsed,'!')),'&amp;&amp;',lyteUiTreeMaxLevelHelp(indexVar,index,ltPropMaxLevel))}}\" is=\"case\" lc-id=\"lc_id_0\"> <lyte-tree class=\"{{lyteTreeClassStateHelp(item)}} lyteTreeChildrenLevel\" lt-prop-children-value=\"{{ltPropChildrenValue}}\" index-var=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\" lt-prop-data=\"{{item[ltPropChildrenValue]}}\" id=\"{{ltPropId}}\" lt-prop-tree-lines=\"{{ltPropTreeLines}}\" lt-prop-yield=\"{{ltPropYield}}\" lt-prop-open-class=\"{{ltPropOpenClass}}\" lt-prop-wrapper-open-class=\"{{ltPropWrapperOpenClass}}\" lt-prop-leaf-node-class=\"{{ltPropLeafNodeClass}}\" lt-prop-close-class=\"{{ltPropCloseClass}}\" lt-prop-wrapper-close-class=\"{{ltPropWrapperCloseClass}}\" lt-prop-max-level=\"{{ltPropMaxLevel}}\" lt-prop-ignore-max-level=\"{{ltPropIgnoreMaxLevel}}\" lt-prop-toggle-animation=\"{{ltPropToggleAnimation}}\" lt-prop-css-level-variable=\"{{ltPropCssLevelVariable}}\" lt-prop-sortable=\"{{ltPropSortable}}\" lt-prop-scroll-speed=\"{{ltPropScrollSpeed}}\" lt-prop-allow-external-import=\"{{ltPropAllowExternalImport}}\" on-toggle=\"{{method('onToggle')}}\" on-toggle-end=\"{{method('onToggleEnd')}}\" on-before-open=\"{{method('onBeforeOpen')}}\" on-open=\"{{method('onOpen')}}\" on-before-close=\"{{method('onBeforeClose')}}\" on-close=\"{{method('onClose')}}\"> <template is=\"registerYield\" yield-name=\"content\" from-parent=\"\"></template> </lyte-tree> </template></template> </div> </lyte-tree-body> </div></template></template> </template> </template><template case=\"hierarchy\" is=\"case\"> <template items=\"{{ltPropData}}\" item=\"item\" index=\"index\" is=\"for\" _new=\"true\"> <table class=\"lyteHTreeNode\" cellpadding=\"0\" cellspacing=\"0\" data-level=\"{{lyteUiTreeLevelHelp(indexVar,index)}}\" child-index=\"{{index}}\" data-index=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\"> <tbody><tr> <td class=\"lyteHTreeParentNode\" onclick=\"{{action('openHorizontalTree',this)}}\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(lyteUiTreeLevelHelp(indexVar,index),'>',0)}}\" is=\"case\" lc-id=\"lc_id_0\"> <div class=\"lyteHTreeConnectorLine\"> <div class=\"lyteHTreeVerticalConnectLine {{lyteHTreeTopVline(index,ltPropData)}}\"></div> <div class=\"lyteHTreeVerticalConnectLine {{lyteHTreeBottomVline(index,ltPropData)}}\"></div> </div> <span class=\"lyteHTreeHorizontalBeforeConnectLine\"></span> </template></template> <div class=\"lyteHTreeContentWrap\"> <lyte-yield yield-name=\"content\" list-value=\"{{item}}\"></lyte-yield> </div> <span class=\"lyteHTreeHorizontalAfterConnectLine lyteHTreeHorizontalConnectorHidden\"></span> </td> <td is=\"switch\" lyte-switch=\"true\" l-c=\"true\" _new=\"true\"></td> </tr> </tbody></table> </template> </template><template case=\"data\" is=\"case\"> <div class=\"lyteDataTreeNodeWrap\"> <template _new=\"true\" value=\"{{dataType}}\" is=\"switch\">  <template case=\"array\" is=\"case\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(arrayOpened,'!')}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteDataTreeNode lyteDataTreeArrayNode {{if(lyteTreeArrayOpened,'lyteDataTreeOpened','lyteDataTreeClosed')}}\" onclick=\"{{action('openArray',this)}}\"> <span> [ <template items=\"{{ltPropArrayData}}\" item=\"item\" index=\"index\" is=\"for\" _new=\"true\"> <template _new=\"true\" value=\"{{lyteTreeDataType(item)}}\" is=\"switch\">    <template case=\"array\" is=\"case\"> <span class=\"test\" style=\"text-transform:capitalize;\">{{lyteTreeDataType(item)}}({{lyteTreeGetArrayLength(item)}})</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"object\" is=\"case\"> <span>{...}</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"string\" is=\"case\"> <span class=\"lyteDataTreeNodeText\">'{{item}}'</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"number\" is=\"case\"> <span>{{item}}</span> <span class=\"lyteTreeComma\">,</span> </template></template> </template> ] </span> </div></template><template default=\"\"><div class=\"lyteDataTreeOpened\" onclick=\"{{action('openArray',this)}}\"> <span>Array({{lyteTreeArrayLength}})</span> </div></template></template> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{lyteTreeArrayOpened}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteDataTreeNodeWrap\"> <template items=\"{{ltPropArrayData}}\" item=\"item\" index=\"index\" is=\"for\" _new=\"true\"> <template _new=\"true\" value=\"{{lyteTreeDataType(item)}}\" is=\"switch\">     <template case=\"array\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeArray\"> <span class=\"lyteDataTreeIndex\">{{index}}</span> <span>:</span> <div class=\"lyteDataTreeNodeContent\"> <lyte-tree class=\"lyteDataTreeSubLevel\" lt-prop-structure-type=\"data\" lt-prop-array-data=\"{{item}}\" on-before-open=\"{{method('onBeforeOpen')}}\" on-open=\"{{method('onOpen')}}\" on-before-close=\"{{method('onBeforeClose')}}\" on-close=\"{{method('onClose')}}\"> <template is=\"registerYield\" yield-name=\"content\"></template> </lyte-tree> </div> </div> </template><template case=\"object\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeObject\"> <span class=\"lyteDataTreeIndex\">{{index}}</span> <span>:</span> <div class=\"lyteDataTreeNodeContent\"> <lyte-tree class=\"lyteDataTreeSubLevel\" lt-prop-structure-type=\"data\" lt-prop-json-data=\"{{item}}\" on-before-open=\"{{method('onBeforeOpen')}}\" on-open=\"{{method('onOpen')}}\" on-before-close=\"{{method('onBeforeClose')}}\" on-close=\"{{method('onClose')}}\"> <template is=\"registerYield\" yield-name=\"content\"></template> </lyte-tree> </div> </div> </template><template case=\"number\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeNumber\"> <span class=\"lyteDataTreeIndex\">{{index}}</span> <span>:</span> <span class=\"lyteDataTreeNodeContent lyteDataTreeNodeText\">{{item}}</span> </div> </template><template case=\"string\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeString\"> <span class=\"lyteDataTreeIndex\">{{index}}</span> <span>:</span> <span class=\"lyteDataTreeNodeContent lyteDataTreeNodeText\">{{item}}</span> </div> </template><template case=\"boolean\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeBoolean\"> <span class=\"lyteDataTreeIndex\">{{index}}</span> <span>:</span> <span class=\"lyteDataTreeNodeContent lyteDataTreeNodeText\">{{item}}</span> </div> </template></template> </template> </div></template></template> </template><template case=\"object\" is=\"case\"> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{expHandlers(jsonOpened,'!')}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteDataTreeNode lyteDataTreeObjectNode {{if(lyteTreeJsonOpened,'lyteDataTreeOpened','lyteDataTreeClosed')}}\" onclick=\"{{action('openJson',this)}}\"> <span>{ <template object=\"{{ltPropJsonData}}\" value=\"value\" key=\"key\" is=\"forIn\" _new=\"true\"> <span>{{key}}</span>: <template _new=\"true\" value=\"{{lyteTreeDataType(value)}}\" is=\"switch\">     <template case=\"array\" is=\"case\"> <span style=\"text-transform:capitalize;\">{{lyteTreeDataType(value)}}({{lyteTreeGetArrayLength(value)}})</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"object\" is=\"case\"> <span>{...}</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"string\" is=\"case\"> <span class=\"lyteDataTreeNodeText\">'{{value}}'</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"number\" is=\"case\"> <span class=\"lyteDataTreeNumber\">{{value}}</span> <span class=\"lyteTreeComma\">,</span> </template><template case=\"boolean\" is=\"case\"> <span class=\"lyteDataTreeBoolean\">{{value}}</span> <span class=\"lyteTreeComma\">,</span> </template></template> </template> }</span> </div></template><template default=\"\"><div class=\"lyteDataTreeOpened\" onclick=\"{{action('openJson',this)}}\"> <span class=\"lyteDataTreeObjectTextLabel\">Object</span> </div></template></template> <template is=\"switch\" l-c=\"true\" _new=\"true\"><template case=\"{{lyteTreeJsonOpened}}\" is=\"case\" lc-id=\"lc_id_0\"><div class=\"lyteDataTreeNodeWrap\"> <template object=\"{{ltPropJsonData}}\" value=\"value\" key=\"key\" is=\"forIn\" _new=\"true\"> <template _new=\"true\" value=\"{{lyteTreeDataType(value)}}\" is=\"switch\">     <template case=\"array\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeArray\"> <span class=\"lyteDataTreeIndex\">{{key}}</span> <span>:</span> <div class=\"lyteDataTreeNodeContent\"> <lyte-tree class=\"lyteDataTreeSubLevel\" lt-prop-structure-type=\"data\" lt-prop-array-data=\"{{value}}\" on-before-open=\"{{method('onBeforeOpen')}}\" on-open=\"{{method('onOpen')}}\" on-before-close=\"{{method('onBeforeClose')}}\" on-close=\"{{method('onClose')}}\"> <template is=\"registerYield\" yield-name=\"content\"></template> </lyte-tree> </div> </div> </template><template case=\"object\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeObject\"> <span class=\"lyteDataTreeIndex\">{{key}}</span> <span>:</span> <div class=\"lyteDataTreeNodeContent\"> <lyte-tree class=\"lyteDataTreeSubLevel\" lt-prop-structure-type=\"data\" lt-prop-json-data=\"{{value}}\" on-before-open=\"{{method('onBeforeOpen')}}\" on-open=\"{{method('onOpen')}}\" on-before-close=\"{{method('onBeforeClose')}}\" on-close=\"{{method('onClose')}}\"> <template is=\"registerYield\" yield-name=\"content\"></template> </lyte-tree> </div> </div> </template><template case=\"string\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeString\"> <span class=\"lyteDataTreeIndex\">{{key}}</span> <span>:</span> <span class=\"lyteDataTreeNodeContent lyteDataTreeNodeText\">{{value}}</span> <span class=\"lyteTreeComma testend\">,</span> </div> </template><template case=\"number\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeNumber\"> <span class=\"lyteDataTreeIndex\">{{key}}</span> <span>:</span> <span class=\"lyteDataTreeNodeContent lyteDataTreeNodeText\">{{value}}</span> <span class=\"lyteTreeComma testend\">,</span> </div> </template><template case=\"boolean\" is=\"case\"> <div class=\"lyteDataTreeNode lyteDataTreeBoolean\"> <span class=\"lyteDataTreeIndex\">{{key}}</span> <span>:</span> <span class=\"lyteDataTreeNodeContent lyteDataTreeNodeText\">{{value}}</span> <span class=\"lyteTreeComma testend\">,</span> </div> </template></template> </template> </div></template></template> </template></template> </div> </template></template> </template>";;
LyteTreeComponent._dynamicNodes = [{"t":"a","p":[1]},{"t":"s","p":[1],"c":{"file":{"dN":[{"t":"a","p":[1],"cn":"file"},{"t":"f","p":[1],"dN":[{"t":"s","p":[1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[0,1],"a":{"style":{"name":"style","helperInfo":{"name":"lyteUiTreeLevelVariableHelp","args":["indexVar","index","ltPropCssLevelVariable"]}}},"cn":"lc_id_0"},{"t":"a","p":[0,1,1],"cn":"lc_id_0"},{"t":"a","p":[0,1,1,1],"cn":"lc_id_0"},{"t":"s","p":[0,1,1,1],"c":{"withExternal":{"dN":[{"t":"a","p":[1],"cn":"withExternal"},{"t":"a","p":[1,1],"cn":"withExternal"},{"t":"i","p":[1,1],"in":0,"cn":"withExternal"}]},"onlyInternal":{"dN":[{"t":"a","p":[1,1],"cn":"onlyInternal"},{"t":"i","p":[1,1],"in":0,"cn":"onlyInternal"}]},"nonSortable":{"dN":[{"t":"a","p":[1],"cn":"nonSortable"},{"t":"i","p":[1],"in":0,"cn":"nonSortable"}]}},"d":{},"dc":{"withExternal":{"dc":[0],"hc":true,"trans":true},"onlyInternal":{"dc":[0],"hc":true,"trans":true},"nonSortable":{"dc":[0],"hc":true,"trans":true}},"co":["withExternal","onlyInternal","nonSortable"],"hc":true,"trans":true,"in":2,"sibl":[1],"cn":"lc_id_0"},{"t":"s","p":[0,1,1,3],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1],"cn":"lc_id_0"},{"t":"r","p":[1,1],"dN":[],"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[0,1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[2,1,0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0}],"dc":[0],"hc":true,"trans":true,"in":0,"cn":"file"}]},"hierarchy":{"dN":[{"t":"a","p":[1],"cn":"hierarchy"},{"t":"f","p":[1],"dN":[{"t":"a","p":[1]},{"t":"a","p":[1,1,0,1]},{"t":"s","p":[1,1,0,1,1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1,1],"cn":"lc_id_0"},{"t":"a","p":[1,3],"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{}},"hd":true,"co":["lc_id_0"],"in":2,"sibl":[1]},{"t":"a","p":[1,1,0,1,3,1]},{"t":"i","p":[1,1,0,1,3,1],"in":1,"sibl":[0]},{"t":"s","p":[1,1,0,3],"c":{"lc_id_0":{"dN":[{"t":"a","p":[1,1],"cn":"lc_id_0"},{"t":"r","p":[1,1,1],"dN":[],"in":1,"sibl":[0],"cn":"lc_id_0"},{"t":"cD","p":[1,1],"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"actualTemplate":"<template is=\"switch\"><template case=\"{{expHandlers(expHandlers(item.collapsed,'!==',undefined),'&amp;&amp;',expHandlers(item.collapsed,'!==',true))}}\" is=\"case\" lc-id=\"lc_id_0\" depth=\"3\"><table><tbody><tr> <td class=\"lyteHtreeChildNode\"> <lyte-tree id=\"{{ltPropId}}\" class=\"lyteHTreeChildWrap\" index-var=\"{{lyteUiTreeIndexHelp(indexVar,index)}}\" lt-prop-children-value=\"{{ltPropChildrenValue}}\" lt-prop-data=\"{{item[ltPropChildrenValue]}}\" lt-prop-open-class=\"open\" lt-prop-close-class=\"close\" lt-prop-yield=\"{{ltPropYield}}\" lt-prop-structure-type=\"hierarchy\" lt-prop-leaf-node-class=\"{{ltPropLeafNodeClass}}\" on-before-open=\"{{method('onBeforeOpen')}}\" on-open=\"{{method('onOpen')}}\" on-before-close=\"{{method('onBeforeClose')}}\" on-close=\"{{method('onClose')}}\"> <template is=\"registerYield\" yield-name=\"content\" from-parent=\"\"></template> </lyte-tree> </td> </tr></tbody></table></template></template>","in":0}],"dc":[1,0],"hc":true,"trans":true,"in":0,"cn":"hierarchy"}]},"data":{"dN":[{"t":"a","p":[1,1],"cn":"data"},{"t":"s","p":[1,1],"c":{"array":{"dN":[{"t":"s","p":[1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[0],"cn":"lc_id_0"},{"t":"a","p":[0,1,1],"cn":"lc_id_0"},{"t":"f","p":[0,1,1],"dN":[{"t":"a","p":[1]},{"t":"s","p":[1],"c":{"array":{"dN":[{"t":"tX","p":[1,0],"cn":"array"},{"t":"tX","p":[1,2],"cn":"array"}]},"object":{"dN":[]},"string":{"dN":[{"t":"tX","p":[1,1],"cn":"string"}]},"number":{"dN":[{"t":"tX","p":[1,0],"cn":"number"}]}},"d":{},"dc":{"array":{},"object":{},"string":{},"number":{}},"co":["array","object","string","number"]}],"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[0],"cn":"default"},{"t":"tX","p":[0,1,1],"cn":"default"}]},"dc":{"lc_id_0":{},"default":{}},"hd":true,"co":["lc_id_0"],"in":1,"sibl":[0],"cn":"array"},{"t":"s","p":[3],"c":{"lc_id_0":{"dN":[{"t":"a","p":[0,1],"cn":"lc_id_0"},{"t":"f","p":[0,1],"dN":[{"t":"a","p":[1]},{"t":"s","p":[1],"c":{"array":{"dN":[{"t":"tX","p":[1,1,0],"cn":"array"},{"t":"a","p":[1,5,1],"cn":"array"},{"t":"r","p":[1,5,1,1],"dN":[],"in":1,"sibl":[0],"cn":"array"},{"t":"cD","p":[1,5,1],"in":0,"cn":"array"}]},"object":{"dN":[{"t":"tX","p":[1,1,0],"cn":"object"},{"t":"a","p":[1,5,1],"cn":"object"},{"t":"r","p":[1,5,1,1],"dN":[],"in":1,"sibl":[0],"cn":"object"},{"t":"cD","p":[1,5,1],"in":0,"cn":"object"}]},"number":{"dN":[{"t":"tX","p":[1,1,0],"cn":"number"},{"t":"tX","p":[1,5,0],"cn":"number"}]},"string":{"dN":[{"t":"tX","p":[1,1,0],"cn":"string"},{"t":"tX","p":[1,5,0],"cn":"string"}]},"boolean":{"dN":[{"t":"tX","p":[1,1,0],"cn":"boolean"},{"t":"tX","p":[1,5,0],"cn":"boolean"}]}},"d":{},"dc":{"array":{"dc":[0],"hc":true,"trans":true},"object":{"dc":[0],"hc":true,"trans":true},"number":{},"string":{},"boolean":{}},"co":["array","object","number","string","boolean"],"hc":true,"trans":true,"in":0}],"dc":[0],"hc":true,"trans":true,"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0,"cn":"array"}]},"object":{"dN":[{"t":"s","p":[1],"c":{"lc_id_0":{"dN":[{"t":"a","p":[0],"cn":"lc_id_0"},{"t":"a","p":[0,1,1],"cn":"lc_id_0"},{"t":"fI","p":[0,1,1],"dN":[{"t":"tX","p":[1,0]},{"t":"a","p":[3]},{"t":"s","p":[3],"c":{"array":{"dN":[{"t":"tX","p":[1,0],"cn":"array"},{"t":"tX","p":[1,2],"cn":"array"}]},"object":{"dN":[]},"string":{"dN":[{"t":"tX","p":[1,1],"cn":"string"}]},"number":{"dN":[{"t":"tX","p":[1,0],"cn":"number"}]},"boolean":{"dN":[{"t":"tX","p":[1,0],"cn":"boolean"}]}},"d":{},"dc":{"array":{},"object":{},"string":{},"number":{},"boolean":{}},"co":["array","object","string","number","boolean"]}],"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{"dN":[{"t":"a","p":[0],"cn":"default"}]},"dc":{"lc_id_0":{},"default":{}},"hd":true,"co":["lc_id_0"],"in":1,"sibl":[0],"cn":"object"},{"t":"s","p":[3],"c":{"lc_id_0":{"dN":[{"t":"a","p":[0,1],"cn":"lc_id_0"},{"t":"fI","p":[0,1],"dN":[{"t":"a","p":[1]},{"t":"s","p":[1],"c":{"array":{"dN":[{"t":"tX","p":[1,1,0],"cn":"array"},{"t":"a","p":[1,5,1],"cn":"array"},{"t":"r","p":[1,5,1,1],"dN":[],"in":1,"sibl":[0],"cn":"array"},{"t":"cD","p":[1,5,1],"in":0,"cn":"array"}]},"object":{"dN":[{"t":"tX","p":[1,1,0],"cn":"object"},{"t":"a","p":[1,5,1],"cn":"object"},{"t":"r","p":[1,5,1,1],"dN":[],"in":1,"sibl":[0],"cn":"object"},{"t":"cD","p":[1,5,1],"in":0,"cn":"object"}]},"string":{"dN":[{"t":"tX","p":[1,1,0],"cn":"string"},{"t":"tX","p":[1,5,0],"cn":"string"}]},"number":{"dN":[{"t":"tX","p":[1,1,0],"cn":"number"},{"t":"tX","p":[1,5,0],"cn":"number"}]},"boolean":{"dN":[{"t":"tX","p":[1,1,0],"cn":"boolean"},{"t":"tX","p":[1,5,0],"cn":"boolean"}]}},"d":{},"dc":{"array":{"dc":[0],"hc":true,"trans":true},"object":{"dc":[0],"hc":true,"trans":true},"string":{},"number":{},"boolean":{}},"co":["array","object","string","number","boolean"],"hc":true,"trans":true,"in":0}],"dc":[0],"hc":true,"trans":true,"in":0,"cn":"lc_id_0"}],"cdp":{"t":"a","p":[0]},"dcn":true}},"d":{},"dc":{"lc_id_0":{"dc":[0],"hc":true,"trans":true}},"hd":true,"co":["lc_id_0"],"hc":true,"trans":true,"in":0,"cn":"object"}]}},"d":{},"dc":{"array":{"dc":[0],"hc":true,"trans":true},"object":{"dc":[0],"hc":true,"trans":true}},"co":["array","object"],"hc":true,"trans":true,"in":0,"cn":"data"}]}},"d":{},"dc":{"file":{"dc":[0],"hc":true,"trans":true},"hierarchy":{"dc":[0],"hc":true,"trans":true},"data":{"dc":[0],"hc":true,"trans":true}},"co":["file","hierarchy","data"],"hc":true,"trans":true,"in":0},{"type":"dc","trans":true,"hc":true,"p":[0]}];;

LyteTreeComponent._observedAttributes = [
  "ltPropData",
  "ltPropJsonData",
  "ltPropArrayData",
  "ltPropContent",
  ...
  "ltPropMaxLevel",
  "ltPropStructureType",
  "ltPropToggleAnimation",
  "ltPropSortable",
  "ltPropScrollSpeed",
  "ltPropAllowExternalImport",
  "ltPropIgnoreMaxLevel",
  "ariaLiveContent",
  "ariaLiveBoolean",
  "treeHeight",
  "ltPropStateAttr",
  "ltPropCssLevelVariable",
  "dataType",
  "lyteTreeArrayOpened",
  "lyteTreeJsonOpened",
  "lyteTreeArrayLength",
...
if (!window._lyteUiUtils.registeredCustomElements['lyte-tree-icon']) {
  window._lyteUiUtils.registeredCustomElements['lyte-tree-icon'] = true

  class LyteTreeIconCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
    connectedCallback() {
      var _this = this.$node
      var lyteSelf = _this.component;
      if($L(_this) && !$L(_this).hasClass('lyteTreeClonedIcon')){
          var res = true;
          var classComponent = $L(_this).closest('lyte-tree')[0]
          var classComponentData = classComponent.getData()
          var lyteTreeIcon = _this;
          var iconCorresTree = $L(_this).closest('.lyteTreeMainContainer').find('lyte-tree')[0]
          ...
          if ($L(_this).closest('.lyteTreeMainContainer').hasClass('lyteTreeHasChild') ||
            $L(_this).closest('.lyteTreeMainContainer').hasClass('lyteTreeMaxedChild')) {
          ...
          if(classComponent.getData('ltPropStructureType') !== 'horizontal'){
            ...
            _this.addEventListener('click' , function(eve){
              ...
              var iconClassList = $L(_this).attr('class').split(" ");
              var res;
              ...
              function lyteTreePromiseFun() {
                ...
                if (path.length < 2) {
                  ...
                  if (x !== undefined) {
                    lyteSelf.$addon.objectUtils(x, 'add', 'collapsed', false);
                    if(x.hasChild){
                      lyteSelf.$addon.objectUtils(x, 'add', 'hasChild', false);
                    }
                  }
                } else {
                  var x = treeDt;
            ...
            $L(_this).addClass('lyteHTreeIcon')
  
            if(!classComponent.getData('ltPropData')[$L(_this).closest('table').attr('child-index')][classComponentData.ltPropChildrenValue]){
              $L(_this).addClass(classComponentData.ltPropLeafNodeClass)
            }
  
          }
      }
      var currTreeElem = $L(_this).closest('lyte-tree');
      if (currTreeElem.hasClass('lyteTreeClosed')) {
        currTreeElem[0].style.display = 'none';
      }
      if (!_this.hasAttribute('lyte-custom-icon')) {
        _this.innerHTML = '<i class="arrow up"></i>';
      }
    }
  ...
  LyteTreeIconCustomElements.options = {clone : {allCallbacks : false}};
  LyteTreeIconCustomElements.register("lyte-tree-icon");
}

if (!window._lyteUiUtils.registeredCustomElements['lyte-tree-content']) {
  window._lyteUiUtils.registeredCustomElements['lyte-tree-content'] = true;

  class LyteTreeContentCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
    static get observedAttributes() {
      return [];
    }
    ...
    connectedCallback() {
      var _this = this.$node;
      var currentTree = $L(_this).closest('lyte-tree')[0]
      if(currentTree.getData('ltPropSortable')){
        $L(_this).addClass('lyteTreeSortableElem')
      }
      if(currentTree.getData('ltPropStructureType') !== 'horizontal'){
        var level = $L(_this).closest('lyte-tree-body')[0].getAttribute('data-index').split(' ').length;
        _this.setAttribute('lyte-tree-level', level)
      }
    }
  ...
  LyteTreeContentCustomElements.options = {clone : {allCallbacks : false}};

  LyteTreeContentCustomElements.register("lyte-tree-content");
}

if (!window._lyteUiUtils.registeredCustomElements['lyte-tree-tr']) {
  window._lyteUiUtils.registeredCustomElements['lyte-tree-tr'] = true;

  class LyteTreeTrCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
    static get observedAttributes() {
      return [];
    }
  ...
  LyteTreeTrCustomElements.options = {clone : {allCallbacks : false}};

  LyteTreeTrCustomElements.register("lyte-tree-tr");
}

if (!window._lyteUiUtils.registeredCustomElements['lyte-tree-td']) {
  window._lyteUiUtils.registeredCustomElements['lyte-tree-td'] = true;

  class LyteTreeTdCustomElements extends LyteUiComponentComponentRegistry.CustomElements {
    static get observedAttributes() {
      return [];
    }
    ...
    constructor() {
      ...
      console.log('td');
      if(lyteNode){
        if($L(lyteNode)[0].hasAttribute('lyteTreeSticky')){
          $L(lyteNode).addClass('lyteTreeFixedCol')
        }
      }
    }
  ...
  LyteTreeTdCustomElements.options = {clone : {allCallbacks : false}};

  LyteTreeTdCustomElements.register("lyte-tree-td");
}
 ...
 *	 </lyte-tree>
 */
export { LyteTreeComponent };

LyteTreeComponent.register("lyte-tree", {
  hash: "LyteTreeComponent_4",
