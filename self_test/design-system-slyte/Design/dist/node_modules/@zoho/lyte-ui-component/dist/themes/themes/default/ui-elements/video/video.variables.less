@cssMaskIconLyteVideoIconsCommonColor: #fff;
...
.lyteVideoControlsMixin() {
	@bottom: 0;
	@w: 100%;
	@bg: linear-gradient(to bottom, rgba(0, 0, 0, 0), #000);
	@padding: 20px 30px;
	@opacity: 0;
...
.lyteVideoIconsMixin() {
	@border: 0;
	@outline: 0;
	@bg-img: @lyteVideoSprite;
	@mask-img: @bg-img;
	@bg-color: @cssMaskIconLyteVideoIconsCommonColor;
}
...
.lyteCircleInnerLoaderAfterMixin() {
	...
	@border-radius: 50%;
	@animation: lyteCircleInnerLoaderAfterAnim 1.2s cubic-bezier(0.77, 0, 0.175, 1.000) infinite;
}
