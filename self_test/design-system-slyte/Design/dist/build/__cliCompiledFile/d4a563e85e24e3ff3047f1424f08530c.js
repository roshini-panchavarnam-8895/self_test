...
//                                  this.establishBindings(item._propBindingObject, value);
    ...
    "set" : function(object, property, value, options, fromStore, fromParent, FromUtils, skipTypeError) {
        ...
        if(typeof property === "string" && val1 === val2) {
...
class LyteYield extends HTMLElement {
    ...
    _disconnectedCallback(){
        if(_LC.shouldIgnoreDisconnect() || this._deleted) {
            return;
        }
...
class customElementPrototype extends elementPrototype {
    ...
    afterConnected(fastRenderProp,ssrBind) {
        ...
        var appScp = this.$app || this.$component.$app || this.$component.$addon;
...
//              this.updateYield(updateHelpers[i], false, contextSwitchInfo);
    ...
    updateYield(node, update, contextSwitchInfo,helperNode) {
        ...
        let toAppend = node._callee._yields[_LC.getYieldName(node)];
        if(!toAppend) {
            if(Lyte.Compatibility.vue){
...
//            			}
                ...
                if((dynamicValues.length > 1 || helperFunc._t) && !(_LC.freeze && this.component.__data[firstBoundValue] && this.component.__data[firstBoundValue].freeze )) {
...
//                 	}
                ...
                if(!this.isEmptyString(args[i])) {
                    if(args[i].startsWith("'") && args[i].endsWith("'")){
                        ...
                        if(args[i] === "event" && event) {
                            ...
                            let dynamicVals = [];
                            args[i] = _LC.get(scpData,args[i],dynamicVals,cache);
    ...
    processHelper(scope,helperFunc, node, processDetails){
        ...
        let helperObj = {compClass : scope.$component.registeredHelpers[helperName]};
        if(!helperObj.compClass){
            let obj = {compName : helperName, type : "helper"};
            // _LC.getCompRegistry(obj,this);
            _LC.traverseRegistries([scope.$component], obj, this);
            if(!_LC.verifyDetails(obj)){
                //@Slicer.developmentStart
                RegistryError.error(scope, "LC010", "Helper", helperName);
                //@Slicer.developmentEnd
                return;
            }else{
    ...
    _disconnectedCallback(){
        if(_LC.shouldIgnoreDisconnect() || !this.component || this.__lyteIgnore) {
	       return;
        }
        let shadowObj = this.getShadowObj();
        let directiveObj = this.getDirectiveObj();
...
defHelpers["unescape"] = function(value, additionalObject, userInstance, options) {
    ...
    if(options && options.ignoreSanitizer){
        ...
        let clean;
        let ins = userInstance && Object.keys(userInstance).length ? userInstance._sanitzer : this.$component._ourSanitizerInstance_._sanitzer;
...
export {ComponentRegistry,Component,RawComponent,Helper,arrayUtils,objectUtils,set,appendChild,insertAfter,insertBeforeFn as insertBefore,replaceWith,shouldIgnoreDisconnect,defProp,render,_LC,customElementPrototype,LyteCustomElement,getNearestApp,_addLyteAction,nextTick};
