...
import { cmpData,nestScp,isEntity, deepCopyObject, establishObjectBinding, checkProperty, newGetSuperClass, getSuperClass, defProp, removeNestScp} from "../../core/src/lyte-utils.js";
...
function gE(db, def, pKey, isDeleted, prx){
	...
	if(isDeleted === true){
		...
		if(obj && obj.data){
			if(prx && db.$app.$mutate){
				return db.$app.$mutate.create(obj.data);
			}
			else{
				return obj.data;
			}
		...
		if(!isComp && def.data._recMap){
			...
			if(prx && db.$app.$mutate){
				return db.$app.$mutate.create(rec);
			}
			return rec;
		}
		else{
			...
			if(entity[0]){
				if(prx && db.$app.$mutate){
					return db.$app.$mutate.create(entity[0]);
				}
				else{
					return entity[0];
				}
	...
	if(ins){
		if(ins[key]){
			...
			//@Slicer.catchAllErrorsStart
				...
				db.$app && db.$app.appError ? db.$app.appError.error(db,err):Dberror.error(db,err);
			}
			 //@Slicer.catchAllErrorsEnd
			// if(db.debug){
...
function getInd(data,pKey,pkVal,type){
	for(var i=0;data && i<data.length;i++){
		...
		if(rec[pKey] == pkVal || (isEntity(rec) && rec.$.get && rec.$.get(pKey) == pkVal)){
			return i;
		}
	}
	return -1;
